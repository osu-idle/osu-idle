...
export interface PlayHistory {
	bestRank: Map<number, number>;   // beatmapId -> best Grades index (lower = better)
	lastPlayed: Map<number, number>; // beatmapId -> most recent playedAt (ms epoch)
}

export const EMPTY_HISTORY: PlayHistory = { bestRank: new Map(), lastPlayed: new Map() };
...
/** order sentinel that sinks a catch-all group (#, Never, Unplayed) to the end */
const LAST = 1e9;
...
function letterGroup(value: string): Group {
	...
	return isLetter ? { key: c, label: c, order: c.charCodeAt(0) } : { key: '#', label: '#', order: LAST };
}
...
function lengthGroup(ms: number): Group {
	...
	if (min < 5) return { key: `l${min}`, label: t`${min}-${min + 1} min`, order: min };
	if (min < 10) return { key: 'l5', label: t`5-10 min`, order: 5 };
	return { key: 'l10', label: t`10+ min`, order: 10 };
}

/** Named recency buckets: Today / Yesterday / Last week / N months ago (≈30-day
 *  months) up to "Over 5 months ago", and "Never" for maps with no local play. */
function recencyGroup(last: number | undefined, now: number): Group {
	if (!last) return { key: 'never', label: t`Never`, order: LAST };
	const days = Math.floor((now - last) / DAY);
	if (days <= 0) return { key: 'today', label: t`Today`, order: 0 };
	if (days === 1) return { key: 'yesterday', label: t`Yesterday`, order: 1 };
	if (days < 7) return { key: 'week', label: t`Last week`, order: 2 };
	const months = Math.max(1, Math.floor(days / 30));
	if (months > 5) return { key: 'old', label: t`Over 5 months ago`, order: 99 };
	return { key: `m${months}`, label: plural(months, { one: '# month ago', other: '# months ago' }), order: 2 + months };
}
...
export function groupOf(
	...
	switch (mode) {
		...
		case 'By Difficulty': {
			...
			return { key: `d${n}`, label: `${n} - ${n + 1}★`, order: n };
		}
		case 'By BPM': {
		...
		case 'By Rank Achieved': {
			...
			return { key: Grades[rank], label: Grades[rank], order: rank };
		}
