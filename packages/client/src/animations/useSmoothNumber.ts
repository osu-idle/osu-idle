...
import { SmoothNumber, SmoothNumberOptions } from './smoothNumber';
...
/**
 * React binding for {@link SmoothNumber}: pass the target value and get back the
 ...
 *   const shown = useSmoothNumber(score, { duration: 3000, from: 0 }); // count up
 */
export default function useSmoothNumber(target: number, options?: UseSmoothNumberOptions): number {
	const ref = useRef<SmoothNumber | undefined>(undefined);
	if (!ref.current) ref.current = new SmoothNumber(options?.from ?? target, options);
