...
import { desktop, type DesktopUpdateStatus } from '@osu-idle/shared/desktop';

/**
 * In-game self-update state for the desktop app. We check the update feed on
 * launch (and again whenever the server-version poll in globals.ts notices a new
 * build mid-session), which drives the {@link DesktopUpdate} UI: the player
...
export async function checkForDesktopUpdate(): Promise<DesktopUpdateStatus | undefined> {
	const app = desktop();
	if (!app) return;
...
/** Begin downloading the available update; progress arrives via {@link updateStatus}. */
