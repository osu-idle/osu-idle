...
/** Decode a fixture chart by filename (cached; each run builds its own note runtime). */
export function loadBeatmap(file: string): Beatmap {
	let beatmap = cache.get(file);
	if (!beatmap) {
		beatmap = decoder.decodeFromString(readFileSync(CHARTS_DIR + file, 'utf8'));
		cache.set(file, beatmap);
	}
	return beatmap;
}
