...
import { LoadingPanel, Transition } from './Transition';
 ...
 * and resolves the play session *behind* the cover (and surfaces any errors there).
 */
export function launchPlay(beatmap: LightBeatmap, debug = false): void {
	if (launching) return;
	launching = true;
	const transition = Transition.begin(
		<LoadingPanel title={`${beatmap.set.metadata.artist} - ${beatmap.set.metadata.title}`} sub={beatmap.metadata.version} />,
	);
