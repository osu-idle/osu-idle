...
export async function uploadNewsImage(file: File): Promise<string> {
	...
	const res = await fetch(apiUrl(`${base}/image`), {
...
export const getNewsById = (id: number) => unwrap(rpc.v1.news.admin[':id']
...
export const updateNews = (id: number, body: NewsUpdateBody) =>
	unwrap(rpc.v1.news[':id'].$patch({
		param: { id: String(id) }, json: body, 
	}));
export const deleteNews = (id: number) => unwrap(rpc.v1.news[':id']
